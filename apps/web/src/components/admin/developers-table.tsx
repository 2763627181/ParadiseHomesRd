    ...
    <div className="overflow-hidden rounded-xl border border-border/70">
      <table className="w-full text-sm">
        <thead className="bg-secondary/60 text-xs uppercase tracking-wide text-muted-foreground">
          <tr>
            <th className="px-4 py-2.5 text-left font-medium">Desarrolladora</th>
            <th className="px-4 py-2.5 text-left font-medium">Proyectos</th>
          ...
          {rows.map((row) => (
            <tr key={row.id} className="align-top hover:bg-secondary/30">
              ...
              <td className="px-4 py-3 text-xs text-muted-foreground">
                {row.projectCount} proyecto{row.projectCount === 1 ? "" : "s"}
              </td>
              <td className="px-4 py-3">
                {row.isVerified ? (
              ...
              <td className="px-4 py-3">
                <div className="flex items-center justify-end">
                  <Button asChild variant="ghost" size="icon-sm" title="Ver perfil público">
                    <Link href={`/developers/${row.slug}`} target="_blank">
