...
import { updateAgencyProfile } from "@/lib/actions/agency-profile";
import type { AgencyProfileRow } from "@/lib/data/agency-dashboard";
...
export function AgencySettingsForm({
  ...
  const upload = async (
    ...
    try {
      ...
      console.error("[agency-settings upload]", err);
    ...
    <form onSubmit={submit} className="max-w-2xl space-y-5">
      {readOnly && (
        <p className="rounded-lg border border-warning/30 bg-warning/10 px-3 py-2 text-sm text-warning-foreground">
          Inicia sesión con una cuenta de administrador de la inmobiliaria para editar este perfil.
        </p>
      )}
      ...
      <div className="space-y-1.5">
        ...
        <Textarea
          ...
          disabled={readOnly}
          placeholder="Cuéntales a los clientes sobre tu inmobiliaria…"
        />
      </div>
