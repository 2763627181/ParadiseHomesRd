...
import { getVisitsForAgency } from "@/lib/data/visits";
import { DashboardShell, type DashboardNavItem } from "@/components/dashboard/dashboard-shell";
import { VisitsAgenda } from "@/components/dashboard/visits-agenda";
import { Badge } from "@/components/ui/badge";

export const metadata: Metadata = { title: "Visitas · Inmobiliaria", robots: { index: false } };
...
export default async function AgencyVisitsPage() {
  ...
  const visits = membership ? await getVisitsForAgency(agencyId) : [];

  return (
    <DashboardShell title="Inmobiliaria" nav={NAV}>
      <div className="mb-1 flex items-center gap-2">
        <h1 className="text-xl font-semibold tracking-tight">Visitas</h1>
        {!membership && <Badge variant="warning">Vista de ejemplo</Badge>}
      </div>
      <p className="mb-5 text-sm text-muted-foreground">
        Todas las visitas de tu equipo: solicitudes, confirmaciones e historial.
      </p>
      <VisitsAgenda visits={visits} showAgent canManage={Boolean(membership)} />
    </DashboardShell>
  );
}
