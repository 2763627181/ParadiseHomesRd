...
import { closingStatCards, getClosingsForAgency, summarizeClosings } from "@/lib/data/closings";
...
export const metadata: Metadata = { title: "Cierres · Inmobiliaria", robots: { index: false } };
...
export default async function AgencyClosingsPage() {
  ...
  const rows = membership ? await getClosingsForAgency(agencyId) : [];
  ...
  return (
    <DashboardShell title="Inmobiliaria" nav={NAV}>
      ...
      <p className="mb-5 text-sm text-muted-foreground">Ventas y alquileres cerrados por tu equipo.</p>
